          ...
          <div class="form-control" style="grid-column: 1 / -1">
            ...
            >
              ...
              <template v-if="!fileRef">
                ...
                <div class="drop-zone__hint">支持 JPG、PNG、WebP、PDF，最大 {{ maxUploadSizeMB }}MB</div>
              </template>
              <template v-else>
                <div class="drop-zone__icon">✓</div>
...
const pluginConfig = computed(() => getPluginConfig('callsign-review'))
const maxUploadSizeMB = computed(() => Math.max(1, Number(pluginConfig.value.maxUploadSizeMB || 10)))
const maxUploadBytes = computed(() => maxUploadSizeMB.value * 1024 * 1024)
const noticeText = computed(() => pluginConfig.value.customNotice || '请上传业余无线电操作证书或执照扫描件（PDF / 图片），建议对证件号等敏感信息添加水印或打码处理后再上传。文件大小不超过 10MB。')
const autoUppercaseCallsign = computed(() => pluginConfig.value.autoUppercaseCallsign !== false)
...
function setFile(file) {
  ...
  if (file.size > maxUploadBytes.value) {
    showToast(`文件大小请控制在 ${maxUploadSizeMB.value}MB 以内`)
    return
  }
  fileRef.value = file
}
...
async function submit() {
  const normalizedCallsign = autoUppercaseCallsign.value ? String(form.callsign || '').toUpperCase() : form.callsign
  ...
  try {
    ...
    form.callsign = normalizedCallsign
