  ...
  <section class="page">
    ...
    <div v-else-if="image" class="image-layout" :data-image-slug="image.slug" :data-image-id="image.id">
      <div class="image-viewer">
        ...
        <div v-if="pluginImageActions.length" class="plugin-action-panel">
          <div class="plugin-action-panel__header">
            <div class="eyebrow">插件操作</div>
            <span class="muted">{{ pluginImageActions.length }} 项扩展</span>
          </div>
          <div class="plugin-action-row">
            <mdui-button
              v-for="action in pluginImageActions"
              :key="action.id"
...
const pluginImageActions = computed(() => {
  if (!image.value) return []
  return getImageDetailActions({
    image: image.value,
    auth,
    route,
    router,
...
async function handlePluginAction(action) {
  if (!action || typeof action.onClick !== 'function' || !image.value) return
  try {
...
async function submitEdit() {
  ...
  try {
    ...
    let phashHex = image.value.phash || null
    let md5Hash = image.value.file_md5 || null
    ...
    if (editFile.value) {
      ...
      try {
        const [phashBinary, md5Result] = await Promise.all([
          computePHash(editFile.value),
          computeMD5(editFile.value),
        ])
