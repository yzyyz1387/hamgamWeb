  ...
  <section class="page">
    <mdui-card class="section-card">
      ...
      <NotificationTemplateGallery
        v-if="notificationTemplates.length"
...
import { getNotificationTemplates } from '@/plugins/runtime'
import { applyNotificationTemplateValues } from '@/plugins/templates/notificationTemplates'
import AppTextField from '@/components/form/AppTextField.vue'
import NotificationTemplateGallery from '@/components/admin/NotificationTemplateGallery.vue'
...
function applyTemplate(template) {
  if (!template) return
  const resolved = applyNotificationTemplateValues(template, {
    actor_name: auth.displayName || auth.user?.email || '管理员',
  })
  const values = resolved.resolvedValues || {}
  if (values.target) {
    notificationType.value = values.target
  ...
  if (typeof values.title === 'string') {
    notificationTitle.value = values.title
  }
  if (typeof values.content === 'string') {
    notificationContent.value = values.content
  }
