    ...
    <div v-if="admins.length" class="team-section">
      ...
      <div class="team-admins">
        <div v-for="u in admins" :key="u.id" class="team-card team-card--admin" @click="goUser(u.uid)">
          ...
          <div v-if="getUserExtras(u).length" class="team-card__plugin-extras">
            <component
              :is="extra.component"
              v-for="extra in getUserExtras(u)"
              :key="`${u.id}-${extra.id}`"
              :auth="auth"
              :profile="u"
              :config="extra.config"
              :plugin="extra"
              class="team-card__plugin-extra"
            />
          </div>
        </div>
      </div>
...
import { getUserListItemExtras } from '@/plugins/runtime'
...
function getUserExtras(profile) {
  return getUserListItemExtras({
    auth,
    profile,
    user: profile,
    route: { path: '/team' },
